...
const people = [
    ...
    const inventor1500s = inventors.filter(inventor => inventor.year >= 1500 && inventor.year < 1600);
    ...
    /* Return value of sort() dictates sorting order 
        - Negative if param1 < param2
        - 0 if param1 == param2
        - Positive param1 > param2
    */
    ...
    // 5. Sort the inventors by years lived, add age property for proof
    const inventorSortedByAge = inventors.sort((a, b) => {
    ...
    const deOnly = links.filter(link => link.includes("de"));
    console.log(deOnly);
    ...
    const result = data.reduce((obj, str) => {
        if(!obj[str]) {
            obj[str] = 0;
        }
        obj[str]++;
